...
def find_neediest_day(pool):
    ...
    day_num = ist_now.weekday() + 1
    print(f"Today is {ist_now.strftime('%A')} ({DAY_NAMES[day_num]})")
    return day_num
...
def generate_new_mantra(target_day, existing_sanskrit_openers):
    ...
    if not raw_text:
        print("Failed to generate mantra: Could not get response from Gemini API.")
        return None
    text = raw_text.strip()
    ...
    try:
        ...
        print(f"Failed to parse mantra JSON: {e}")
        return None
...
def generate_ai_image(prompt, topic_keyword="mantras", filename="mantra_image.webp"):
    ...
    return generate_hd_featured_image(prompt, category="Mantras & Chants", output_filename=filename)
...
def main():
    ...
    if not new_mantra:
        print("ERROR: Could not generate a fresh, non-duplicate mantra. Exiting with failure code 1.")
        sys.exit(1)
